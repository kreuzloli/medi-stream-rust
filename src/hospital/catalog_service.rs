...
pub async fn list_departments(
    ...
    let departments = catalog_repository::find_active_departments(&state.db).await?;
    ...
    let disease_map =
        catalog_repository::find_active_disease_map_by_departments(&state.db, &dept_ids).await?;
...
pub async fn list_diseases_by_department(
    ...
    let diseases =
        catalog_repository::find_active_diseases_by_department(&state.db, dept_id).await?;
